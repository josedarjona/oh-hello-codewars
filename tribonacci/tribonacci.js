...
// starting with [0, 1] instead of [1, 1] basically shifts the common
//  Fibonacci sequence by once place, you may be tempted to think that we 
//  would get the same sequence shifted by 2 places, but that is not the case
//   and we would get:

// [0, 0, 1, 1, 2, 4, 7, 13, 24, ...]
// Well, you may have guessed it by now, but to be clear: you need to create
//  a fibonacci function that given a signature array/list, returns the first n 
//  elements - signature included of the so seeded sequence.

// Signature will always contain 3 numbers; n will always be a 
// non-negative number; if n == 0, then return an empty array and be ready 
// for anything else which is not clearly specified ;)

function tribonacci(signature,n){
  //your code here
  
  //setting the signature, the beggining
  var tribo = [...signature]
  console.log(tribo)
  ...
  //checks to make sure the n is <3, for what test ask for
  if(n<3){
